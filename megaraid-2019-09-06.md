...
在这个模型里，**SCSI device和LUN是等价的；host和initiator是等价的**。一个target有一到多个port（transport end point），包含一到多个LUN。例如一个SAS disk就是一个具体的（相对于模型）target，有两个port，包含一个LUN；FC disk也一样。HBA卡或MegaRAID卡是具体的（相对于模型）initiator。
