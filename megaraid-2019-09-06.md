...
在这个模型里，**SCSI device和LUN是等价的；host和initiator是等价的**。一个target有一到多个port（transport end point），包含一到多个LUN。例如一个SAS disk（或一个FC disk）就是一个具体的（相对于模型）target，有两个port，包含一个LUN；MegaRAID卡创建出的Virtual Drive也是一个具体的（相对于模型）target。HBA卡或MegaRAID卡是具体的（相对于模型）initiator。
