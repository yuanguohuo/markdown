...
- Adapter: 即物理MegaRAID卡，物理MegaRAID控制器。一个server上可能有多个adapter，`MegaCli64 -adpCount`返回其数量。`-a`用于指定一个（`-aN`，`N`为adapter的ID）或者所有（`-aALL`）adapter。几乎所有常用命令都需要`-a`选项。一般情况下，server上只有一个adapter，其ID一般为0，这时`-a0`或`-aALL`是等价的。Adapter相关的操作以`-Adp`开头，例如，列出所有adapter的信息：`MegaCli64 -AdpAllInfo -aALL`；只列出adapter-0的信息：`MegaCli64 -AdpAllInfo -a0`。
