...
* 这个左值的类型是右值引用类型；
* 只能付给它右值(从上例可以看出，虽然赋值时一定要是右值，但赋值完成之后，右值变成了左值)；

为什么这样呢？其实这是C++11的一个规则：**右值引用，有名字的是左值；没有名字的是右值。**
...
**左值的引用肯定是左值，但右值的引用却不一定是右值，这取决于它有没有名字**。重申一遍：**右值引用，有名字的是左值；没有名字的才是右值。**背后的动机是什么呢？因为一个右值，一经拷贝(无论是做拷贝构造函数的参数，还是出现在"="的右边)，便失去了它的资源(被move走了)。假如存在有名字的右值，一个不经意的拷贝(在"="右边)，就把它修改了，让它丢失资源从而变成变成空壳子。这会带来严重的后果：想想我们看代码的时候，要找一个变量在哪里被修改过，都是找它在"="左边的出现，不是吗？谁会关心它在"="右边的出现呢？也就是说，假如存在有名字的右值，我们要找它在哪里被修改，就不得不注意它所有的出现。如例2中，f在"="右边出现一次，就不是原来的f了。这非常不合理。
