...
右值引用和左值引用的不同之处：
* 初始化不同；右值引用只能用右值进行初始化，而左值引用必须用左值(这里指非const情形)。

也就是说：**右值引用类型的变量和左值引用类型的变量，除了初始化不同之外，其他都是相同的**(对吗？还有什么不同？)
...
"typename remove_reference<T>::type"就是把T中的引用去掉，无论T是"Foo&"还是"Foo"，它的结果都是Foo。这样就明确了，std::move()就是通过static_cast<>把参数t强制为右值的。

另外，注意std::move的参数t，它是一个右值引用吗？如果是，那么std::move的参数只能是右值。这显然不对啊，因为std::move的作用就是把左值强制为右值，所以它必须能够接受左值作参数。事实上，t不是右值引用，而是一个**通用引用**。关于通用引用，我将在[C++11中的通用引用](http://www.yuanguohuo.com/2018/05/25/cpp11-universal-ref/)中介绍。
