...
虽然"&&"很容易让人想起**右值引用**，但它不是！你给它左值，它表现出左值引用的行为；给它右值，它表现出右值引用的行为。这种灵活的引用，叫作**通用引用**。
...
也就是说，不存在类型推导的"&&"是右值引用，例如：
```cpp
void func(Foo&& param); //无类型推导，所以是右值引用，不是通用引用
Foo&& r = Foo();        //无类型推导，所以是右值引用，不是通用引用
```

注意，下面这种，虽然有类型推导，但也不是通用引用，因为推导的类型并不是"&&"的类型：
```cpp
template<typename T>
void func(std::vector<T>&& param); //右值引用，不是通用引用  
```

通用引用在**完美转发**中扮演着重要的角色。
...
# 小结
本文介绍了C++11引入的auto关键字，包括类型推导规则、使用场景。和它类似、又相关的decltype，将在下一篇介绍。
