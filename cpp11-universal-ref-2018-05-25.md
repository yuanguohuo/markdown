...
虽然我们不能在源代码里写*引用的引用*(如4.2节所示)，但是由于第4.1节的类型推导规则(下面例1和例2)，模板类型参数实例化(下面例3)和typedef展开(下面例4)等原因，**在编译的过程中**，可能产生*引用的引用*这种情况：
...
如第4.2节所说，*引用的引用*是非法的。那我们这些例子中都存在*引用的引用*怎么办呢？好在，这些*引用的引用*不是存在于源代码中(否则编译失败)，而是在编译过程中临时产生的。编译器会立即消除它们，手段就是**引用折叠(reference collapsing)**。
