...
熟悉右值引用的话，就会知道右值引用是通过`&&`来声明的。可能看见`&&`的第一反应就是右值引用。可惜，有些情况下竟然不是。在[C++11中的右值引用](http://www.yuanguohuo.com/2018/05/25/cpp11-rvalue-ref/)，我们看到了`std::move`的源代码：
...
也就是说，`std::move`也可以接受右值作参数，t也能够匹配右值。实际上，参数t就是一个**通用引用**，**可以使用左值表达式初始它，也可以使用右值表达式初始化它。匹配左值表达式时，它表现的像一个左值引用；匹配右值表达式时，它表现的像一个右值引用**。
...
也就是说，若T是已知类型，不需要推导，则是右值引用；T需要推导，则是通用引用。先看一些例子：
...
而下面这种情况，却是有类型推导的(调用`Bar()`和`emplace_back()`的时候，需要推导类型)，是通用引用；
...
首先，auto和模板类型T的推导规则是这样的：

* 若匹配的是右值，`auto`或`T`被推导成那个右值的类型；例如匹配的是3，那么推导结果是`int`；
* 若匹配的是左值，`auto`或`T`被推导成那个左值的引用类型；例如匹配的是`i`(`int i=3;`)，那么推导的结果是`int&`；

## 引用的引用是非法的 (4.2)

其次，必须说明，代码中是不能有*引用的引用*这样的类型的，例如：
...
上述编译错误的后半句指出，它不是一个typedef或者模板类型参数。实际上，在**编译过程中**，由于类型推导规则(4.1节)和typedef展开等，可能产生*引用的引用*这种情况。所以需要编译器进行引用折叠(reference collapsing)。
...
根据类型推导规则：
* 对于`func(3)`，T的推导结果是`int`，模板实例化的结果是`void func(int&& p)`；
* 对于`func(i)`，T的推导结果是`int&`，模板实例化的结果是`void func(int& && p)`；

对于后一种情况，模板函数实例化的结果中存在*引用的引用*，是非法的，需要引用折叠。
...
根据类型推导规则：
* r1引用的是右值，所以auto的推导结果是`int`，r1的类型是`int&&`；
* r2引用的是左值，所以auto的推导结果是`int&`，r2的类型是`int& &&`；

r2的类型是*引用的引用*，也需要引用折叠。
...
这个例子和类型推导规则(4.1节)无关。当你用引用类型去实例化Bar的时候，就出现了*引用的引用*：

```cpp
Bar<int&> b1;
Bar<int&&> b2;
```
如此，模板类实例化就包含了引用的引用，需要折叠：

```cpp
typedef int& & LVRefType;   //b1的类
typedef int&& & LVRefType;  //b2的类
```
...
* 例2：r2引用的是左值，所以auto的推导结果是`int&`，r2的类型是`int& &&`，折叠成`int&`，通用引用r2最终等价于左值引用；
* 例3：`int& &`和`int&& &`都折叠成`int&`，就是说LVRefType最终的类型是`int&`；这么定义是正确的：无论取代T的是不是引用，是左值引用还是右值引用，最终LVRefType都是左值引用类型；
* 例4：`int& && p`被折叠成`int& p`；
...
1. 在func(r1)中，p的类型是`int&& &&`，然后折叠成`int&&`吗？不是。在推导类型T的时候，r1的右值引用是被剥除的。又因为r1是一个左值(说过很多次了，右值引用类型的变量有名字，因此是左值，见[C++11中的右值引用](http://www.yuanguohuo.com/2018/05/25/cpp11-rvalue-ref/))，所以T的推导结果是`int&`。所以，模板实例化的结果是`void func(int& && p)`，折叠结果是`void func(int& p)`。
2. 在func(r2)中，p的类型是`int& &&`，然后折叠成`int&`吗？是的。但是，这和`int& r2=x`中的`&`无关，因为它也被剥除了。T被推导成`int&`完全是因为r2是左值的缘故。

可见，由于引用剥除的缘故，r1和r2声明中的`&&`和`&`起不到任何作用；模板参数类型的推导，完全取决于函数实参是左值还是右值。引用折叠则取决于类型推导结果(若结果是引用，则需要折叠)。

# 小结 (5)

本文主要介绍通用引用，并由此引起了引用折叠(reference collapsing)以及引用剥除(reference stripping)等话题。通用引用在完美转发中起着重要作用，见[C++11中的完美转发]()。
